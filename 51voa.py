...
def parse_page_list(url):
    ...
    soup = BeautifulSoup(req.text, "html5lib")
    try:
        alinks = soup.select('div#pagelist > a')
        print len(alinks)
        for link in alinks:
            nextUrl = urlparse.urljoin(url, link['href'])
            print 'parse_page_list:'+nextUrl
            parse_title_list(nextUrl)
...
def parse_list(url):
    ...
    soup = BeautifulSoup(req.text, "html5lib")
    try:
        alinks = soup.select('div#left_nav > ul > li > a')
        print len(alinks)
        for link in alinks:
            nextUrl = urlparse.urljoin(url, link['href'])
            print 'parse_list:'+nextUrl
            parse_page_list(nextUrl)
