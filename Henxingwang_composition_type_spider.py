...
if __name__ == '__main__':
    # type = [('yingyusijizuowen',34),('CET6',17),('gaozhongyingyuzuowen',56),('chuzhongyingyuzuowen',63),('xiaoxueyingyuzuowen',80)]
    type = [('chuzhongyingyuzuowen',50),('xiaoxueyingyuzuowen',80)]
    for item in type:
        size = item[1]
        for i in range(1,size+1,1):
            if i == 1:
                url = 'http://www.hxen.com/englishwriting/%s/' % item[0]
            else:
                url = 'http://www.hxen.com/englishwriting/%s/index_%d.html'% (item[0],i)
            print('root url:'+url)
            get_all_link(url)
